...
router.post('/signup', async (request, response) => {
  try {
    ...
    const userObject = {
      ...
      displayName: request.body.username.trim(),
    ...
    const verificationLink = `https://animazingcatalogue.netlify.app/verify-email/${verificationToken}`
...
router.put('/forgotpassword/', async (request, response) => {
  ...
  try {
    ...
    if (user) {
      // Used to generate a random string and new time and store it
      const verificationString = crypto.randomBytes(32).toString('hex')
      user.resetToken = verificationString
      user.resetTokenExpiry = new Date()
      try {
        const verificationLink = `https://animazingcatalogue.netlify.app/forgotpassword/${verificationString}`
